...
for i in range(len(objpoints)):
    ...
    error_left = cv.norm(imgpointsL[i], imgpoints2, cv.NORM_L2) / len(imgpoints2)
    mean_error_left += error_left

print("Reprojection error for left camera: {}".format(mean_error_left / len(objpoints)))
...
for i in range(len(objpoints)):
    ...
    error_right = cv.norm(imgpointsR[i], imgpoints2, cv.NORM_L2) / len(imgpoints2)
    mean_error_right += error_right

print("Reprojection error for right camera: {}".format(mean_error_right / len(objpoints)))
...
retStereo, newCameraMatrixL, distL, newCameraMatrixR, distR, rot, trans, essentialMatrix, fundamentalMatrix = \
    cv.stereoCalibrate(objpoints, imgpointsL, imgpointsR, newCameraMatrixL, distL, newCameraMatrixR, distR,
...
rectL, rectR, projMatrixL, projMatrixR, Q, roi_L, roi_R = \
    cv.stereoRectify(newCameraMatrixL, distL, newCameraMatrixR, distR, grayL.shape[::-1], rot, trans, rectifyScale,
                     (0, 0))
